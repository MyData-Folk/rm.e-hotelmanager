...
def calculate_plan_from_rule(base_price: float, rule, steps: list) -> dict:
    ...
    rounded_result = apply_rounding(
        raw_result,
        getattr(rule, "rounding_mode", "two_decimals") or "two_decimals",
    ...
    return {
        ...
        "rounding_mode": getattr(rule, "rounding_mode", "two_decimals") or "two_decimals",
        "rounding_increment": getattr(rule, "rounding_increment", None),
        "rounding_scope": getattr(rule, "rounding_scope", "final") or "final",
        "trace": trace,
    }
